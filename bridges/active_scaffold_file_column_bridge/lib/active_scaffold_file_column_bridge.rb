...
module ActiveScaffold
  module Helpers
    ...
    module ListColumns
      def render_list_column_with_file_column(record, column)
        if column_override?(column) || !active_scaffold_config.file_column_fields.include?(column.name.to_sym)
          return render_list_column_without_file_column(record, column)
        end
      ...
      alias_method_chain :render_list_column, :file_column
    end
  end
end
